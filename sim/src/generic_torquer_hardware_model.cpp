...
namespace Nos3
{
    ...
    Generic_torquerHardwareModel::Generic_torquerHardwareModel(const boost::property_tree::ptree& config) : SimIHardwareModel(config), _enabled(0), _count(0), _config(0), _status(0)
    {
    ...
    void Generic_torquerHardwareModel::command_callback(NosEngine::Common::Message msg)
    {
        ...
        else if (command.substr(0,7).compare("STATUS=") == 0)
        {
            try
            {
                ...
                response = "Generic_torquerHardwareModel::command_callback:  Status set";
            }
            catch (...)
            {
                response = "Generic_torquerHardwareModel::command_callback:  Status invalid";
            }            
        }
    ...
    /* Custom function to prepare the Generic_torquer HK telemetry */
    void Generic_torquerHardwareModel::create_generic_torquer_hk(std::vector<uint8_t>& out_data)
    {
        /* Prepare data size */
    ...
    /* Custom function to prepare the Generic_torquer Data */
    void Generic_torquerHardwareModel::create_generic_torquer_data(std::vector<uint8_t>& out_data)
    {
        boost::shared_ptr<Generic_torquerDataPoint> data_point = boost::dynamic_pointer_cast<Generic_torquerDataPoint>(_generic_torquer_dp->get_data_point());
    ...
    void Generic_torquerHardwareModel::uart_read_callback(const uint8_t *buf, size_t len)
    {
        ...
        std::uint8_t valid = GENERIC_TORQUER_SIM_SUCCESS;
        ...
        if (_enabled != GENERIC_TORQUER_SIM_SUCCESS)
        {
            sim_logger->debug("Generic_torquerHardwareModel::uart_read_callback:  Generic_torquer sim disabled!");
            valid = GENERIC_TORQUER_SIM_ERROR;
        }
        else
        {
            ...
            if (valid == GENERIC_TORQUER_SIM_SUCCESS)
            {   
                /* Process command */
                switch (in_data[2])
                {
                    case 0:
                        /* NOOP */
                        sim_logger->debug("Generic_torquerHardwareModel::uart_read_callback:  NOOP command received!");
                        break;

                case 1:
                        /* Request HK */
                        sim_logger->debug("Generic_torquerHardwareModel::uart_read_callback:  Send HK command received!");
                        create_generic_torquer_hk(out_data);
                        break;

                    case 2:
                        /* Request data */
                        sim_logger->debug("Generic_torquerHardwareModel::uart_read_callback:  Send data command received!");
                        create_generic_torquer_data(out_data);
                        break;
                    ...
                    default:
                        /* Unused command code */
                        valid = GENERIC_TORQUER_SIM_ERROR;
                        sim_logger->debug("Generic_torquerHardwareModel::uart_read_callback:  Unused command %d received!", in_data[2]);
                        break;
                }
            }
        }

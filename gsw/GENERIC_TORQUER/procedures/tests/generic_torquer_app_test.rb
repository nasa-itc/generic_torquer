...
GENERIC_TORQUER_TEST_LOOP_COUNT.times do |n|
    ...
    get_generic_torquer_hk()
    check("GENERIC_TORQUER GENERIC_TORQUER_HK_TLM CMD_COUNT == 0")
    check("GENERIC_TORQUER GENERIC_TORQUER_HK_TLM CMD_ERR_COUNT == 0")
end
...
GENERIC_TORQUER_TEST_LOOP_COUNT.times do |n|
    # Bad length
    cmd_cnt = tlm("GENERIC_TORQUER GENERIC_TORQUER_HK_TLM CMD_COUNT")
    cmd_err_cnt = tlm("GENERIC_TORQUER GENERIC_TORQUER_HK_TLM CMD_ERR_COUNT")
    cmd("GENERIC_TORQUER GENERIC_TORQUER_NOOP_CC with CCSDS_LENGTH #{n+2}") # Note +2 due to CCSDS already being +1
    get_generic_torquer_hk()
    check("GENERIC_TORQUER GENERIC_TORQUER_HK_TLM CMD_COUNT == #{cmd_cnt}")
    check("GENERIC_TORQUER GENERIC_TORQUER_HK_TLM CMD_ERR_COUNT == #{cmd_err_cnt+1}")
end
